...
float uniform_rand(int fineness = RAND_MAX) {
...
template<typename T> bool linear_search(T* begin, size_t size, T elem) {
...
void transform_pts_3(float* points, int num_pts, mat3 F) {
	int n = 3;
	for (int i = 0; i < num_pts; i++) {
		vec3 p(points + i * n);
		p = F * p;
